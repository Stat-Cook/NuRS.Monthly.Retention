...
load.pas.data <- function(sql.table= "pas_wardstays"){
  ...
  pas.ws <- tbl(pkg.env$con, sql.table) %>% 
    collect()
  colnames(pas.ws) <- c("Hospital_Provider_Spell_No", "PatientID", 
...
make.admissions.discharges <- function(sql.table= "pas_wardstays"){
  ...
  pas.end.frame <- pas.ws %>% filter(Ward %in% names(pas_to_allocate_list)) %>%#
    mutate(Ward = pas_to_allocate(Ward)) %>% 
...
make.ward.occupancy <- function(sql.table= "pas_wardstays"){
  ...
  lagged.pas.f <- function(window) lagged.group(pas.per.bed, "Month Starting", window) %>% 
