make.incident.data <- function(sql.table="jpuh_ulysses_IncidentFile"){
  ...
  harmful <- c("2 - Minor/Non Permanent Harm", "3 - Moderate/Semi Permanent Harm",
  ...
  incident.summary.f <- function(window) {
    ...
    pas.days <- pas.days.occupied %>% lagged.group("Month Starting", window) %>%
      summarize(`Days Used` = sum(`Days Used`))

    left_join(inc, pas.days) %>% mutate(
