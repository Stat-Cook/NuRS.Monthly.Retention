...
make.monthly.assignment <- function(fn = pkg.env$default_functions){
  ...
  assignment.metrics.f <- process.across.f(assignment.shift.overlap.per.bed, "Date",
  ...
  between.var.f <- process.across.f(var.data$averages, "Date",
                                    where(is.numeric),
                                    list("Between Shift Var" = var))
  
  within.var.f <- process.across.f(var.data$`De-meaned`, "Date",
                                   where(is.numeric),
